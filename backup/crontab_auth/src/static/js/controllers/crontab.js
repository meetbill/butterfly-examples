...
app.controller('CrontabCtrl', ['$scope', '$filter', '$http', 'editableOptions', 'editableThemes',
  function($scope, $filter, $http, editableOptions, editableThemes){
    ...
    //];
    $http.get("/api/get_jobs").then(
        function successCallback(response) {$scope.jobs = response.data.data;},
       // function errorCallback(response) { console.log(response.data);alert(response.data.err_info) },
    )

    // remove job
    $scope.removeJob = function(name) {
        var remove_job = $http.post("/api/remove_job",{
                "name":name}
                )
    ...
    $scope.saveJob = function(data) {
      ...
      return $http.post('/api/add_job', data);
    };
  }]);
